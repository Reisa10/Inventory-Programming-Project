...
def list_products():
    ...
    for row in ws1.iter_rows(min_row=2, values_only=True):
        ...
        print(f"{pid:<10} {name:<20} {cat:<15} {price:<10} {stock:<10} {reorder:<10}")
...
def list_sales():
    ...
    for row in ws2.iter_rows(min_row=2, values_only=True):
        ...
        if sid is None:
            continue
        print(f"{sid:<10} {date:<20} {pid:<10} {qty:<5} {price:<10} {total:<10}")
...
def list_inventory_movements():
    ...
    for row in ws4.iter_rows(min_row=2, values_only=True):
        ...
        if mid is None:
            continue
        print(f"{mid :<5} {pid :<10} {mtype :<10} {safe_int(qty):<5} {date :<20} {remarks :<20}")
